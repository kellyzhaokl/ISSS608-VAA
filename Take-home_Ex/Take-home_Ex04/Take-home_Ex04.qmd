...
rainfall_avg <- data_2023 %>%
  group_by(Month, Day) %>%
  summarise(avg_rainfall = mean(DailyRainfall, na.rm = TRUE), .groups = "drop")

ggplot(rainfall_avg, aes(x = factor(Day), y = factor(Month))) +
  geom_tile(aes(fill = avg_rainfall), color = "white") +
  scale_fill_viridis(na.value = "white", name = "Avg Rainfall (mm)") +
  labs(x = "Day", y = "Month") +
  theme_minimal() +
